...
export OVPN_FILE=$(echo "${OVPN_FILE}" | sed -e 's~^[ \t]*~~;s~[ \t]*$~~')
...
if [[ $VPN_ENABLED == "yes" ]]; then
	...
	export VPN_CONFIG=$(find /config/openvpn -maxdepth 1 -name "${OVPN_FILE}.ovpn" -print -quit)
	
	# if ovpn file not found in /config/openvpn then exit
	if [[ -z "${VPN_CONFIG}" ]]; then
		echo "[crit] No OpenVPN config file located in /config/openvpn/${OVPN_FILE}.ovpn, please download from your VPN provider, rename it to something easy, such as US_California.ovpn,no spaces, and make sure filename matches variable set. Place the file in /config/openvpn/ and then restart this container, exiting..." | ts '%Y-%m-%d %H:%M:%.S' && exit 1
	fi
